...
class Link(models.Model):
    ...
    weight = models.PositiveSmallIntegerField(default=1, choices=zip(range(1, 6), range(1, 6)), verbose_name="权重",
                                              help_text="权重高展示顺序靠前")
...
class SideBar(models.Model):
    ...
    title = models.CharField(max_length=50, verbose_name="标题")
    display_type = models.PositiveSmallIntegerField(default=1, choices=SIDE_TYPE, verbose_name="展示类型")
    content = models.CharField(max_length=500, blank=True, verbose_name="内容", help_text="如果设置的不是HTML类型，可为空")

    status = models.PositiveSmallIntegerField(default=STATUS_SHOW, choices=STATUS_ITEMS, verbose_name="状态")
    owner = models.ForeignKey(User, verbose_name="作者")
    created_time = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    ...
    def content_html(self):
        """直接在sidebar模型中渲染模板，再直接在页面中使用该数据，不需要通过view层"""
        # 避免循环引用
